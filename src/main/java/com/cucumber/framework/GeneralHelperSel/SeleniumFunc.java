...
public class SeleniumFunc implements SeleniumFuncLoc {
	...
	/* Method to print the number of windoes present */
	...
	public static void xpath_GenericMethod_Click_By_JavaScript(String xpath) throws Exception {
		try {
			if (driver.findElement(By.xpath(xpath)).isDisplayed()) {
				WebElement ele = driver.findElement(By.xpath(xpath));
				waitForElement(ele, 1);
	...
	public String getDefaultValueFromDD(String locator) throws InterruptedException {

		goToFrameByTag_IdByXpath(locator);
		WebElement ele = driver.findElement(By.xpath(locator));
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].scrollIntoView();", ele);
		Select select = new Select(driver.findElement(By.xpath(locator)));
		WebElement option = select.getFirstSelectedOption();
		String default_selected_option = option.getText();
		System.out.println(default_selected_option);
		driver.switchTo().defaultContent();
		return default_selected_option;

	}

	public static void xpath_GenericMethod_Click(WebElement ele) throws Exception {
		try {
			if (ele.isDisplayed()) {
				waitForElement(ele, 1);
				JavascriptExecutor js = (JavascriptExecutor) driver;
			...
			try {
				String framebytagName = goToFrameByTag_NameByXpath(ele);

				System.out.println("using getattribute name");
				if (framebytagName == null) {
					goToFrameByTag_IdByXpath(ele);
					System.out.println("using getattribute Id");
				}
				waitForElement(ele, 1);
				JavascriptExecutor js = (JavascriptExecutor) driver;
				js.executeScript("arguments[0].click();", ele);// waitFor(1);
				// System.out.println("using getattribute Id");
				// driver.switchTo().defaultContent();
			} catch (Exception e1) {
				System.out.println("switching to default content");
	...
	public static String goToFrameByTag_NameByXpath(WebElement ele) throws InterruptedException {
		System.out.println("In GoToFrames Method Start");
		...
		for (int i = 0; i < frames.size(); i++) {
			...
			try {
				if (ele.isDisplayed()) {
					System.out.println("Element" + ele + "is present in the frame: " + frameNametwo);
					frameNameone = frameNametwo;
					break;
				}
			} catch (Exception e) {
				System.out.println("Element is not present inside the frame: " + ele);
				driver.switchTo().defaultContent();
			}
		}
		System.out.println("In GoToFrames Method End");
	...
	public static String goToFrameByTag_IdByXpath(WebElement ele) throws InterruptedException {
		System.out.println("In GoToFrames Method Start");
		String frameNameone = null;
		List<WebElement> frames = driver.findElements(By.tagName("iframe"));
		System.out.println(frames.size());
