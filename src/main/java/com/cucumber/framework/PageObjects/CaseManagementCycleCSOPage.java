...
public class CaseManagementCycleCSOPage extends SeleniumFunc
	...
	public String[] expectedPolicyStatuses = { "Please Select...", "Open", "Pending Acceptance", "Pending Closing",
			"Open Closing Received", "Open-Processing", "Pending RFI", "Resolved-InForce", "Open Referral", "Lapse",
	...
	public String[] expectedReasonCodes = { "Account number", "Authority Code", "Authority code required",
			"Broker Contact Details", "Clarification on instructions", "COC information", "Completed by UW",
			"Created survey WO", "Duplicate/Existing survey report/request found", "Excess",
			"Forwarded emails from broker", "If policy to be lapsed", "Insured name", "L3 actioned with DTP",
			"L3 actioned with recommendation", "L3 RATER", "Late Triage", "Mismatch information - Closing Check",
			"Missing documents", "Mistriage", "National scheme", "No broker contact", "No existing instructions",
			"Other", "Out of scope", "Per DTP", "Policy number", "Reassigned per DTP", "Rego No",
			"Reviewed eRef for Voice outbound", "State of Risk", "Sublimits", "Sum Insured", "Survey completion date",
			"Vehicle Rate", "Vehicles Details", "Voice Outbound" };
	...
	public String LoginToUserAndGetTheSum(String productCode, String sub) throws Exception {
		if (productCode.equalsIgnoreCase("ISR") || productCode.equalsIgnoreCase("ISC")
			...
			int sum1 = LoginAndGetSumOfUserOne("MVAUser1", "OF@12345");
			System.out.println(sum1);
			int sum2 = LoginAndGetSumOfUserTwo("MVAUser2", "OF@12345");
			System.out.println(sum2);
	...
	public void VerifyTheCaseRoutingAndPriority(String expectedSubject, String expectedFromAddress, String priority)
			throws Exception {
	...
	public void ProvideExpirationReceivedDates(String requestType) throws Exception {
		if (requestType.equalsIgnoreCase("Rnwl Issue Terms B – Email")
			...
			System.out.println("Expiration and Received Data fields are not displayed");
		}
	}
	...
	public void SelectCreateRFI() throws Exception {
		...
		WebElement createRFIHeader = driver.findElement(By.xpath(CreateRFI_header));

		if (createRFIHeader.isDisplayed()) {

			System.out.println("User navigates to Create RFI screen");
			Assert.assertTrue(true);
		} else {
			System.out.println("User failed to navigates to Create RFI screen");
			Assert.assertTrue(false);
		}
	}

	public void ClickOnResonCodesDropdown() throws Exception {

		xpath_GenericMethod_Click(ReasonCodes_dropdown);
	}
	...
	public void VerifyReasonCodesDropDownValues() {
		...
		if (reasoncodes.size() == expectedReasonCodes.length) {
			...
			for (int i = 0; i < reasoncodes.size(); i++) {
				...
				if (reasoncodes.get(i).getText().equalsIgnoreCase(expectedReasonCodes[i])) {

					System.out.println("Actual Reason code is = " + reasoncodes.get(i).getText()
							+ " and Expected Reason Code is = " + expectedReasonCodes[i] + " are macthed");
					Assert.assertTrue(true);
				} else {
					System.out.println("Actual Reason code is = " + reasoncodes.get(i).getText()
							+ " and Expected Reason Code is = " + expectedReasonCodes[i] + " are not macthed");
					Assert.assertTrue(false);
	...
	public void SelectReasonCodeFromDropdown(String reasoncode) {
		...
		for (int i = 0; i < reasoncodes.size(); i++) {
			...
			if (reasoncodes.get(i).getText().equals(reasoncode)) {

				reasoncodes.get(i).click();
			} else {
				System.out.println("Provided reason code is not available in the list");
			}
		}
	...
	public void ClickOnCreateButton() throws Exception {
		...
		xpath_GenericMethod_Click(Create_button);
		waitFor(10);
		WebElement RFINew = driver.findElement(By.xpath("//h1[text()='RFI']//following::span[2]"));

		if (RFINew.isDisplayed()) {
			System.out.println("Create RFI screen is displayed");
			Assert.assertTrue(true);
		} else {
			System.out.println("Create RFI screen not is displayed");
			Assert.assertTrue(false);
		}
	}

	public void VerifyCreatedRFI() throws Exception {

		xpath_GenericMethod_Click(Create_button);

		waitForElement(RFI_AwaitingResponse_badge, 50);

		WebElement RFIAwaitingResponseBadge = driver.findElement(By.xpath(RFI_AwaitingResponse_badge));

		if (RFIAwaitingResponseBadge.isDisplayed()) {
			System.out.println("RFI is Submitted");
			Assert.assertTrue(true);
		} else {
			System.out.println("RFI is not Submitted");
			Assert.assertTrue(false);
		}

	}

	public void NavigateOriginalCase() throws Exception {

		xpath_GenericMethod_Click(OriginalCase_link);
	}

	public void SelectContinueProcess() throws Exception {
		xpath_GenericMethod_Click(ContinueProcess_option);
	}

	public void VerifyCSOCannotProceedeWithoutClosingRFI() {

		WebElement warning = driver.findElement(By.xpath(Warning_msg));
	...
	public void SelectFirstCase() throws Exception {

		xpath_GenericMethod_Click(FirstCase);
	}

	public void VerifySuccessValidation() {

		WebElement tickIcon = driver.findElement(By.xpath(TickMark));
	...
	public void VerifyUnSuccessfullValidation() {

		WebElement crossIcon = driver.findElement(By.xpath(WrongMark));
	...
	public void VerifyPolicyOrQuoteNoAtInformationNote(String number) {
		
		String actualNumber = driver.findElement(By.xpath(PolicyOrQuoteNo_AtInformationTab)).getText();
		
		Assert.assertEquals(actualNumber, number);
		
	}
	
	public void PolicyOrQuoteNoVerificationAtInformationTab() {
		
		WebElement tickIcon = driver.findElement(By.xpath(TickMark_AtInformationTab));
	...
	public void VerifyInvalidPolicyOrQuoteNoErrorMessage() {
		
		WebElement error = driver.findElement(By.xpath(ErrorMessage_PolicyOrQuote));
	...
	public void ClickOnSubmitTheCase() throws Exception {
		
		xpath_GenericMethod_Click(Submit_button);
	}
	
	public void VerifyCaseIsResolvedOrNot() {
		
		WebElement badge = driver.findElement(By.xpath(ResolvedCompleted_badge));
	...
	public void VerifyCaseDetailsAtInfomationTab(String xpath ,String expected) throws InterruptedException {
	...
	public void VerifyTypeOfRequestAtInformationTab(String expectedRequestType) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(TypeOfRequest_AtInformationTab, expectedRequestType);
	}
	
	public void VerifyPolicyStatusAtInformationTab(String expectedPolicyStatus) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(PolicyStatus_AtInformationTab, expectedPolicyStatus);
	}
	
	public void VerifyClaimIDAtInformationTab(String expectedClaimID) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(ClaimId_AtInformationTab, expectedClaimID);
	}
	
	public void VerifyInsuredNameAtInformationTab(String expectedInsuredName) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(InsuredName_AtInformationTab, expectedInsuredName);
	}
	
	public void VerifyUnderWriterNameAtInformationTab(String expectedUnderWriterName) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(UnderWriterName_AtInformationTab, expectedUnderWriterName);
	}
	
	public void VerifyUnderWriterRegionAtInformationTab(String expectedUnderWriterRegion) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(UnderWriterRegion_AtInformationTab, expectedUnderWriterRegion);
	}
	public String VerifyTypeAtInformationTab(String expectedType) throws InterruptedException {
		
		goToFrameByTag_IdByXpath(Type_AtInformationTab);
		
		String actual = driver.findElement(By.xpath(Type_AtInformationTab)).getText();
	...
	public void VerifyPolicyOrQuoteNoAtInformationTab(String type, String expectedNumber) throws InterruptedException {
		
		if(type.equals("policy")) {
			
			VerifyCaseDetailsAtInfomationTab(QuoteNo_AtInformationTab, expectedNumber );
		}else {
			VerifyCaseDetailsAtInfomationTab(QuoteNo_AtInformationTab, expectedNumber );
		}
	...
	public void VerifyProductCodeAtInformationTab(String expectedProductCode) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(ProductCode_AtInformationTab, expectedProductCode);
	}
	
	public void VerifyPriorityAtInformationTab(String expectedPriority) throws InterruptedException {
		
		VerifyCaseDetailsAtInfomationTab(Priority_AtInformationTab, expectedPriority);
	}
